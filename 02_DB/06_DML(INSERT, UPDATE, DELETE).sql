...
-- 작성법 1)  INSERT INTO 테이블명 VALUES(데이터, 데이터, ...)
...
VALUES(900, '장채현', '901123-2345678', 'jang_ch@kh.or.kr',
...
ROLLBACK; -- DML 실행 취소! 데이터 저장한거 삭제!
...
VALUES ('D0', '개발부', 'L1'); -- 1행 삽입

SELECT * FROM DEPARTMENT2; -- 확인

ROLLBACK; -- DML실행 취소

SELECT * FROM DEPARTMENT2; -- 취소확인



---------------------------------------

-- 작성법 2)  INSERT INTO 테이블명(컬럼명, 컬럼명, 컬럼명,...)
...
-- 컬럼명 명시에서는 순서가 바뀌어도 상관 X

SELECT * FROM EMPLOYEE2
WHERE EMP_ID = 900;

-- 잘못 장채현 추가 후 커밋(완전저장)한 경우 삭제 구문
...
-- 서브쿼리로 한번에 기존 데이터들을 여러개 복사붙여넣기도 가능!
...
-- UPDATE 후 위 SELECT 결과 수행 -> 결과 확인 후 ROLLBACK;
...
WHERE EMP_ID  IN (SELECT EMP_ID
				...
				JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
				JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
				WHERE LOCAL_NAME LIKE 'ASIA%'); -- 다중행 서브쿼리
...
SELECT * FROM EMP_M01; 
SELECT * FROM EMP_M02; --EMP_M01과 겹치는 행이 4행, 겹치지 않는 행 1행

UPDATE EMP_M02 SET SALARY = 0; --EMP_M02의 SALARY를 모두 0으로 함
...
UPDATE SET
EMP_M01.EMP_NAME = EMP_M02.EMP_NAME, -- 2번의 값을 1번으로 집어넣겠다
...
DELETE FROM EMPLOYEE2
WHERE EMP_NAME = '장채현';
-- SELECT 절을 다 지우고 DELETE를 쓰면 삭제완료!
...
-- EMPLOYEE2 테이블 전체 삭제
SELECT * FROM EMPLOYEE2; -- 전체조회
DELETE FROM EMPLOYEE2; -- 전체삭제


ROLLBACK; --부활!!! DML(INSERT, UPDATE, DELETE) 실행취소
...
-- ROLLBACK을 통해 복구할 수 없다.(DML이 아니라서) 재가되어 사라짐 ㅋㅋㅋㅋㅋ
-- DELETE는 하나씩 집어 버리는 느낌이라면 TRUNCATE는 걍 뒤집어엎어버림
