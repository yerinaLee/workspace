...
/* 자료형
    ...
    CHAR(크기) : 고정길이 문자형 (2000BYTE) 
        ...
        ﻿(자바의 char는 문자1개, DB에서의 CHAR는 문자열)
...
CREATE TABLE MEMBER (
	-- 컬렴명 자료형 ,
	MEMBER_ID VARCHAR2(20), -- 상황마다 글자수를 유동적으로 조절하고싶다!
...
-- 만든 테이블 확인
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES; --사용자가 만든 테이블들
...
INSERT INTO MEMBER
VALUES ('MEM01', '123ABC', '전원우', '890413-1357613', DEFAULT );
												-- TO_DATE('2023-08-27')
												-- SYSDATE
												-- DEFAULT (=SYSDATE)
...
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PASS, MEMBER_NAME, MEMBER_SSN)
...
/*
    ...
    데이터 무결성 보장을 목적으로 함. (신뢰성이 높은 데이터)
	...
	PRIMARY KEY : (기본키는 주로 사번, 학번같은 것 / 행을 구분하는 값. 이것만 보면 누구인지알수있다!)
	NOT NULL : 컬럼명이 비어있을 수 없다
	UNIQUE : 중복 불가
	CHECK : 지정된 값만 삽입, 수정 가능
	FOREIGN KEY : 다른 테이블의 기본키 또는 UNIQUE가 설정된 컬럼을 참조
				-> 다른 테이블의 값을 가져다 씀
				-> 두 테이블이 같은 값을 가지는 컬럼이 생기게됨
				-> JOIN 가능 =========== 관계 형성
...
-- 컬럼 레벨 제약 조건 설정
--> 테이블 생성 시 컬럼을 지정하는 부분에 이어서 제약조건을 작성하는 경우
...
CREATE TABLE USER_USED_UK(
    ...
    --USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE,
    					...
    					-- 컬럼 레벨 제약 조건(제약조건 이름 지정 O)
    
    USER_ID VARCHAR2(20), -- 컬럼 레벨 제약조건 X
    USER_PWD VARCHAR2(30) ,
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
    PHONE VARCHAR2(30),
    ...
    -- UNIQUE(USER_ID) -- 제약 조건(컬럼명) -> 제약조건명 미지정
...
--> 아이디에 NULL 값 삽입 가능.
-- ORA-00001: 무결성 제약 조건(A230724_YRN.SYS_C0013064)에 위배됩니다
-- SYS_C0013064 : 제약조건 이름 미설정 시 자동으로 지정되는 이름

-- 00001: 무결성 제약 조건(A230724_YRN.USER_ID_U)에 위배됩니다
...
--> USER_NO가 다름
-- ORA-00001: 무결성 제약 조건(A230724_YRN.USER_ID_NAME_U)에 위배됩니다
...
--  USER_NO NUMBER CONSTRAINTS USER_NO_PK PRIMARY KEY, -- 컬럼 레벨, 이름 지정 O
    
	USER_NO NUMBER, -- 테이블 레벨 지정용 컬럼
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(A230724_YRN.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("A230724_YRN"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(A230724_YRN.PK_USERNO_USERID)에 위배됩니다

SELECT * FROM USER_USED_PK2;

-- PRIMARY KEY는 복합키에서 한개라도 NULL이 들어갈 수 없음
INSERT INTO USER_USED_PK2
VALUES(NULL, 'user01', 'pass01', '신사임당', '여', '010-9999-9999', 'sin123@kh.or.kr');
-- ORA-01400: NULL을 ("A230724_YRN"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
	--> 무결성을 지킬 수 있음. 참조는 여러번 할 수 있어도(이건 중복 아님) 참조대상은 1개이기때문에
	--> 특정 값에 대한 중복 제거(무결성 유지)
	--> 두 테이블이 같은 컬럼 값을 지님 ==> JOIN 활용 // 두 테이블의 연관성이 깊어짐

-- FOREIGN KEY제약조건에 의해서 테이블간의 관계(RELATIONSHIP)가 형성됨 // R-DBMS
-- 제공되는 값 외에는 NULL(아무것도 참조 안함!)을 사용할 수 있음
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
--  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK1 -- 제약조건 이름 추가
 ...
 * 
 * FK 제약조건이 설정된 부모-자식 테이블 // FK가 있음으로써 좀더 확실하게 JOIN 가능
 * -> 무조건 JOIN 가능
 * */

-- USER_USED_FK, USER_GRADE 테이블 JOIN

SELECT USER_ID, USER_NAME, GRADE_CODE , GRADE_NAME
FROM USER_USED_FK
...
-- 삭제옵션을 따로 적지 않으면 기본으로 지정됨

-- GRADE_CODE 중 20은 외래키로 참조되고 있지 않으므로 삭제가 가능함.
-- DELETE FROM 테이블명
-- WHERE 조건
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 20; --> 1번에 삭제 성공
...
CREATE TABLE USER_USED_FK2(
  ...
  REFERENCES USER_GRADE2(GRADE_CODE) ON DELETE SET NULL
							/*부모키가 삭제되면 해당 컬럼 값을 NULL로 세팅*/
  
  -- 삭제옵션 중요해요!!!! 이거 제대로 설정 안하면 나중에 골치아파집니다... ^^ 삭제가 1도 안댐
...
SELECT * FROM USER_GRADE2; -- 부모 테이블
SELECT * FROM USER_USED_FK2; -- 자식 테이블

-- 부모 테이블인 USER_GRADE2에서 GRADE_COED =10 삭제
--> ON DELETE SET NULL 옵션이 설정되어 있어 오류없이 삭제됨.
DELETE FROM USER_GRADE2 WHERE GRADE_CODE =10; --삭제 성공!

-- 자식 테이블 조회
SELECT * FROM USER_USED_FK2;
--> 홍길동, 이순신의 GRADE_CODE가 10 -> NULL로 변화 확인


-- 3) ON DELETE CASCADE : 부모키 삭제시 자식키도 함께 삭제됨 // 3대멸망이야뭐야; CASCADE : 작은폭포, 종속
-- 부모키 삭제시 값을 사용하는 자식 테이블의 컬럼에 해당하는 행이 삭제가 됨
-- 많이 사용은 안하지만, 임시로 썼다가 하루이틀 후 지워도 되는 애들때 사용
...
SELECT * FROM USER_GRADE3; -- 10 삭제 확인

-- ON DELETE CASECADE 옵션으로 인해 참조키를 사용한 행이 삭제됨을 확인
SELECT * FROM USER_USED_FK3 ; -- 홍길동, 이순신 삭제(10 참조하던 행 모두 삭제)
...
-- ORA-02290: 체크 제약조건(A230724_YRN.GENDER_CHECK)이 위배되었습니다
...
UPDATE USER_USED_CHECK
SET GENDER = '여자' -- '남'또는 '여'만 들어갈 수 있음
WHERE USER_NO = 1; 
-- ORA-02290: 체크 제약조건(A230724_YRN.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST (
	...
	USER_PWD VARCHAR2(100) CONSTRAINT NN_USER_PWE NOT NULL, -- 비밀번호는 100정도가 좋아요! 암호화하면 용량이 커짐
	PNO 	 CHAR(14)	   CONSTRAINT NN_PNO NOT NULL /*CONSTRAINT UK_PNO UNIQUE*/,
	GENDER   CHAR(3) 	   CONSTRAINT CK_GENDER CHECK(GENDER IN ('남', '여')), -- 한글은 3바이트!
--	GENDER   NVARCHAR2(1), -- (유니코드) 글자수 1개 유(니N)코드할때 N임
	PHONE	 VARCHAR2(20),
	ADDRESS  VARCHAR2(500), --주소가 기니까 500정도
	STATUS	 CHAR(1)       CONSTRAINT NN_STATUS NOT NULL, --한글 아니고 영어니까 1로!
...
INSERT INTO USER_TEST VALUES (1, 'MEM01', '1111', '790426-1234567', '남', '010-1111-1234', '서울시 강남구', 'N');
INSERT INTO USER_TEST VALUES (2, 'MEM02', '222', '890426-1234567', '여', '010-6544-1234', '어디게용', 'N');
INSERT INTO USER_TEST VALUES (3, 'MEM03', '113411', '698426-1234567', '남', '010-1245-1234', '강북삼성', 'Y');
INSERT INTO USER_TEST VALUES (4, 'MEM04', '13411', '541326-1234567', '남', '010-4345-1234', '서울시 서초구', 'N');
INSERT INTO USER_TEST VALUES (5, 'MEM05', '1234111', '54426-1234567', '여', '010-1541-1234', '서울시 강남구', 'N');
INSERT INTO USER_TEST VALUES (6, 'MEM06', '1234111', '52426-1234567', '여', '010-1541-1234', NULL, 'N');
...
--> 뼈대만 복사! 데이터가 없는 빈 테이블로 복사하기
--> 조건식 결과를 무조건 FALSE로 만들면 가능

CREATE TABLE DEPARTMENT_COPY
AS SELECT * FROM DEPARTMENT
--   WHERE DEPT_ID = 'DD'; -- 없는 데이터를 가지고 그대로 복사 ㅋㅋ
	WHERE 1=0; -- 모든 행들이 다 FALSE가 되서 테이블이 안나옴. 이방법 제일 많이 사용
 
SELECT * FROM DEPARTMENT_COPY;



-- 9. 이미 만들어진 테이블에 제약조건 추가
...
CONSTRAINT DEPT_CODE_FK FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT/*(DEPT_ID)*/;
												--> 참조 테이블 컬럼명 생략 -> PK와 연결
